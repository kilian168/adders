...
// "main" entity, generalized from 4 digits to 4). SD_a, SD_b are unsigned
// 4-bit operands and TC is a two's-complement (signed) 4-bit operand.
...
// Sum_a + Sum_b - 2^4 == SD_a + SD_b + TC_signed.
#include <systemc.h>
#include <iostream>
#include <iomanip>
#include <string>
...
//    4 independent full adders, one per digit, with NO carry chain
//    between them. Gate count: 4 x 5 (full adders) + 1 (NOT) = 21
// ==========================================
...
SC_MODULE(Testbench) {
    ...
    void apply_test(unsigned int sda_value, unsigned int sdb_value, unsigned int tc_value) {
        ...
        long long actual_result = static_cast<long long>(to_unsigned_5bit(Sum_a.read()))
                                 + static_cast<long long>(to_unsigned_5bit(Sum_b.read()))
                                 - (1LL << 4);
...
int sc_main(int argc, char* argv[]) {
    ...
    double avg_switches = total_samples > 0
        ...
        : 0.0;
    std::cout << "SDTC-PPM-4-MC: GATES=21 SAMPLES=" << total_samples
