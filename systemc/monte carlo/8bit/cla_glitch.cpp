// cla_glitch.cpp – 8-bit Carry Lookahead Adder (2-level CLA tree), gate-level switching activity
#include <systemc.h>
#include <iostream>
#include <iomanip>
#include <string>
...
//    7 gates — identical to the old flat ClaUnit2 (since a 2-leaf tree
//    has only one node and no split/combine distinction to gain from).
//
...
//    Both 4-bit blocks work in parallel; the ClaTree computes the carry
//    into block 1 and the overall carry out simultaneously from group-P/G
//    signals (which depend only on A and B, not on Cin), rather than
//    wiring block0.Cout directly into block1.Cin.
...
SC_MODULE(CarryLookaheadAdder8) {
    ...
    sc_signal<bool>     c_mid[NBLK - 1];      // carry into block 1 (from ClaTree)
    ...
    SC_CTOR(CarryLookaheadAdder8) {
        ...
        // ---- Second-level ClaTree: computes c_mid[0], Cout from group P/G ----
...
int sc_main(int argc, char* argv[]) {
    ...
    double avg_switches = total_samples > 0
        ...
        : 0.0;
    std::cout << "CLA-8-MC: GATES=83 (2x38 blocks + 7 CLA-tree) SAMPLES=" << total_samples
